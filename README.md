...
This repository includes a comprehensive set of deep-dive analysis reports generated by **Gemini**, covering the core architecture, request flow, and specialized subsystems of Claude Code.

You can find these reports in the [`kickoff_gm/`](./kickoff_gm/) directory:

1.  **[Architecture Analysis](./kickoff_gm/01-工程架构分析.md)**: Deep dive into the layered services, utils, and React/Ink TUI design.
2.  **[Startup Flow](./kickoff_gm/02-启动流程分析.md)**: Details on parallel prefetching (Keychain/MDM), command parsing, and REPL mounting.
3.  **[Request Processing](./kickoff_gm/03-请求处理流分析.md)**: The lifecycle from user input to AI loop and self-healing recovery.
4.  **[Tool System](./kickoff_gm/04-工具系统分析.md)**: Analysis of Zod-driven tool schemas, dynamic UI rendering, and permission controls.
5.  **[Bridge & Remote](./kickoff_gm/05-Bridge远程系统分析.md)**: How SSE/CCR duplex communication and epoch management enable Remote Cowork.
6.  **[State Management](./kickoff_gm/06-状态管理系统分析.md)**: Internal AppState, non-React Store, and TUI performance optimizations.
7.  **[MCP Protocol](./kickoff_gm/07-MCP协议系统分析.md)**: Integration of Model Context Protocol, dynamic capability mapping, and Elicitation.
8.  **[UI & Rendering](./kickoff_gm/08-UI与输出渲染系统分析.md)**: React+Ink TUI implementation, virtual message lists, and theming.
9.  **[Hooks System](./kickoff_gm/09-Hooks钩子系统分析.md)**: Lifecycle interception points, Stop Hooks, and automated quality feedback.
10. **[Context & Compaction](./kickoff_gm/10-上下文管理与压缩系统分析.md)**: Deep dive into Snip, Context Collapse, and AI Summarization pipelines.
