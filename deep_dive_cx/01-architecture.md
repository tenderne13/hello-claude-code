...
如果只看目录名，很容易把它误判成一个“命令行聊天工具”。但从运行结构上看，`src` 更像下面这几层叠加在一起：

1. 一个 CLI 入口与启动编排器。
2. 一个基于 Ink/React 的终端 UI 运行时。
3. 一个自研会话状态系统。
4. 一个支持多轮工具调用的模型执行引擎。
5. 一个工具、命令、技能、插件、MCP、Agent 的可扩展平台。
6. 一个非常重视提示词缓存、上下文压缩、资源回收和并发安全的生产级系统。

所以阅读 `src` 时，最重要的不是背目录，而是先建立“层”和“流”的概念。
...
理解这几个点后，后面再看启动、请求、工具、Agent、MCP，都会顺很多。
